...
public class DoughnutChart extends RoundChart {
    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
        ...
        if (this.mCenterX == ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED) {
            this.mCenterX = (left + right) / 2;
        }
        if (this.mCenterY == ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED) {
        ...
        for (category = 0; category < cLength; category++) {
            ...
            for (i = 0; i < sLength; i++) {
                ...
                drawLabel(canvas2, this.mDataset.getTitles(category)[i], this.mRenderer, prevLabelsBounds, this.mCenterX, this.mCenterY, shortRadius, longRadius, currentAngle, angle, left, right, this.mRenderer.getLabelsColor(), paint, true, false);
                currentAngle += angle;
            }
        ...
        drawLegend(canvas3, this.mRenderer, categories, left, right, y, width, height, legendSize, paint, false);
