    ...
    public void draw(Canvas canvas, int x, int y, int width, int height, Paint paint) {
        ...
        for (i = 0; i < sLength; i++) {
            ...
            if (this.mRenderer.isDisplayValues()) {
                ...
                List<RectF> list = prevLabelsBounds;
                drawLabel(canvas2, getLabel(this.mRenderer.getSeriesRendererAt(i).getChartValuesFormat(), this.mDataset.getValue(i)), this.mRenderer, list, this.mCenterX, this.mCenterY, shortRadius / 2.0f, longRadius / 2.0f, currentAngle, angle, left, right, this.mRenderer.getLabelsColor(), paint, false, true);
            }
            if (loadPieCfg) {
